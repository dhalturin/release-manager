		...
		Func: func() (interface{}, bool) {
			go func() {
				...
				if err != nil {
					...
					if Payload.Callback == "job run repo-list" {
						repoName = Payload.Actions[0].Selected[0].Value
					} else {
						if err := repoListView(false, "job run repo-list", false); err != nil {
							SendError(Error.push(500, 100, err.Error()).join())
							return
						}
					}
				...
				if Payload.Callback == "job run job-list" {
					...
					if job, _, err := git.Jobs.GetJob(project.ID, jobID); err != nil {
						...
						SendError(Error.get([]int{1141}).join())
						return
					}

					job := &gitlab.Job{}
					if out, _, err := git.Jobs.PlayJob(project.ID, jobID); err == nil {
						job = out
					} else {
						if out, _, err := git.Jobs.RetryJob(project.ID, jobID); err == nil {
							job = out
						} else {
							SendError(Error.push(500, 100, err.Error()).join())
							return
					...
					Slack.ResponseDelay(slack.ResponseJSON{
						...
						Attachments: []slack.ResponseAttachment{
							slack.ResponseAttachment{
								Text:  fmt.Sprintf("Job *%s* will be started", job.Name),
								Color: "good",
							},
						},
					})
						...
						for _, job := range jobs {
							actions = append(actions, slack.ResponseAttachmentActions{
								...
								Confirm: &slack.ResponseConfirm{
									Title:       fmt.Sprintf("Run job: %s", job),
									Text:        fmt.Sprintf("Are you sure you want to run the job \"*%s*\"? Current state: *%s*", job, list[stage][job][0]),
									TextDismiss: "No",
									TextOK:      "Yep",
								},
							})
